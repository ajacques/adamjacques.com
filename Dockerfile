...
RUN /usr/bin/apt-get update \
  && /usr/bin/apt-get install -qy --no-install-recommends make g++ ruby ruby-dev libmysqlclient18 libmysqlclient-dev \
  && gem install bundler --no-ri --no-rdoc \
  && /usr/bin/env bundle install --without assets development test \
  && /usr/bin/apt-get remove -qy --purge ruby-dev g++ make patch gcc libmysqlclient-dev \
